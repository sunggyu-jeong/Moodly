...
export async function fetchAIReportMock(dateISO: string): Promise<AIReport> {
  ...
  endpoints: build => ({
    requestAIWeeklySummary: build.mutation<WeeklySummaryResult, WeeklySummaryPayload>({
      query: payload => async (client: SupabaseClient) => {
        const res = await client.functions.invoke('ai-proxy', {
          body: payload,
          headers: { 'Content-Type': 'application/json' },
        });
        return res;
      },
      transformResponse: (response: unknown) => parseMaybeString<WeeklySummaryResult>(response),

      invalidatesTags: ['WeeklyProgress'],
      // fixedCacheKey: 'ai-weekly-summary',
    ...
    getAIReportByDateMock: build.query<AIReport, string>({
      queryFn: async dateISO => {
        const data = await fetchAIReportMock(dateISO);
        return { data };
      },
